...
mass_Zd = int(mass_pair.split("_")[1])
...
f.write(f"define l- = e- mu- \n")
f.write(f"define f+ = u c d~ s~ b~ e+ mu+ ta+ t \n")
f.write(f"define f- = d s u~ c~ b e- mu- ta- t~ \n")
f.write(f"generate g g > h HIG=1 HIW=0 QED=0 QCD=0, (h > z zp, z > l+ l-, zp > f+ f-)\n")
...
f.write(f"set mzdinput {mass_Zd} \n") # Set a mass for the LLP.
f.write(f"set mhsinput 10000 \n") # Set a mass for the LLP.
