...
logger = logging.getLogger('atlas_susy_2018_14_logger')
...
def compute_poi_ul(resultsDict, expected=None,
                   bg_json_file=Path('./ATLAS_data/likelihood/Comb_bkgonly.json')) -> Union[None, float]:
    import spey
    ...
    # Set default value in case the calculation fails:
    mu_UL = 0.0

    lumi = 139.0 # in fb^-1
    xsec_pb = resultsDict.get('Cross-Section (pb)', None)
    if xsec_pb is None:
        return mu_UL
    else:
        xsec = float(xsec_pb)*1e3

    nsignal = xsec*lumi
    signal_yields = {}
    for SR in ['ee','em','mm']:
        signal_yields[f'SR{SR}_cuts'] = float(resultsDict.get(f'AccEffCuts_{SR}', 0))*nsignal
    ...
    statistical_model = pdf_wrapper(analysis="simple_pyhf",
