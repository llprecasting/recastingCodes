/* Toy detector simulation for use with Pythia8
   Authors: Nishita Desai (with input from Jad Marrouche)
            Giovanna Cottin for trackless jet implementation
...
bool ToyDetector::getObjects(const Event& event){
    ...
    particleTemp.set_user_index(i);// needed for tracklessjets
	...
	Particle jetParticle;// for trackless jets
    for(int j = 0; j < sortedJets.size(); j++){
		...
		bool hasTrack = false;
		double ptsum = 0.0;
		vector<fastjet::PseudoJet> constituents = sortedJets[j].constituents();
		for (int i = 0; i < constituents.size(); i++) {
			jetParticle = event[constituents[i].user_index()];
			if (jetParticle.isNeutral()){continue;}
			if (jetParticle.vProd().pT() > 4.){continue;}
			ptsum += jetParticle.pT();
		}
		// cout<<"ptsum = "<<ptsum<<endl;
		if(ptsum > 5.0) hasTrack = true;
		if(!placed) jets.push_back(pnew);
		if(!placed || !hasTrack) tracklessjets.push_back(pnew);
		if(tracklessjets.size() > jets.size()) cout << "Error in trackless jets" << endl; 
...
void ToyDetector::printObjects(){
    if(tracklessjets.size() > 0){
      cout << "Jets (pT, eta, phi)"<< endl;
      for(unsigned long i=0; i<tracklessjets.size(); i++){
	Vec4 obj = tracklessjets.at(i); 
	cout << obj.pT() <<"  "
	     << obj.eta() <<"  "
	     << obj.phi() <<"  "
	     << endl;
