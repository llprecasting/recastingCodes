...
std::vector<DisplacedVertex> getDVs(Event &event){
...
vector <fastjet::PseudoJet> getJets(Event &event, fastjet::JetDefinition jetDef, double pTjMin, double etaMax)
//Cluster jets
{
    //Get jets:
    ...
    for (int i = 0; i < event.size(); ++i) {
        ...
        // Exclude neutrinos and muons
        int pdg = event[i].idAbs();
        if (pdg == 12 || pdg == 14 || pdg == 16 || pdg == 13){continue;}
    ...
    inclusiveJets = clustSeq.inclusive_jets(pTjMin);
