/* Toy detector simulation for use with Pythia8
   ...
   Simple jet reconstruction at generator level. Cuts on truth jets as described in DV+jets ATLAS search in:
   https://atlas.web.cern.ch/Atlas/GROUPS/PHYSICS/PAPERS/SUSY-2018-13/hepdata_info.pdf
   // Giovanna Cottin 
    // --------------------------------------------------
    // LLP and truth jet matching to be done in main code
    // Discarded truth jets that originate from LLPs decaying outside the calorimeter (R > 3870 mm)
    // in both signal regions. To be implemented in main code, as only discarding the truth-match to LLP ones 
    // --------------------------------------------------
    ////////////////////////////////
...
bool ToyDetector::getObjects(const Event& event){
    ...
    // No neutrinos and no muons
      if (event[i].idAbs() == 12 || event[i].idAbs() == 14 ||
          event[i].idAbs() == 16 || event[i].idAbs() == 13)     continue;
